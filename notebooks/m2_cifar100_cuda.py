...
class M2_base(nn.Module):
    def __init__(self, latent_features, classifier, num_labels = 100):
...
class M2(object):
    ...
    def _train(self,train_loader,test_loader,optimizer,epoch,alpha,labeled_data_len):
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            y_onehot = torch.FloatTensor(len(labels),101)
            y_onehot.zero_()
            y_onehot.scatter_(1,labels.view(-1,1),1)
            y_onehot = y_onehot.narrow(1,0,100)
            ...
            index_n = torch.nonzero(labels == 100).reshape(-1)
            index_n = index_n.to(self.device)
            index = torch.nonzero(labels != 100).reshape(-1)
